// server/models/spaceType.model.ts
...
export const spaceTypeModel = {
    ...
    async create(data: { name: string, nameEn?: string, icon?: string }): Promise<SpaceType> {
    ...
    async update(id: number | string, data: { name: string, nameEn?: string, icon?: string }): Promise<SpaceType> {
        const { name, nameEn, icon } = data;
        const res = await pool.query(
