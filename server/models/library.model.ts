// server/models/library.model.ts
...
export interface LibrarySidebarData {
    typeName: string;
    authorName: string;
    [key: string]: unknown;
}
...
export const libraryModel = {
    async getSidebarData(): Promise<LibrarySidebarData[]> {
    ...
    async getFilters(spaceId: number | string | null, currentFilters: LibraryFilterOptions = {}): Promise<LibraryFilters> {
        const { typeId, authorId, topicsPage = 1, topicsLimit = 15 } = currentFilters;

        const baseParams: unknown[] = [];
        let baseWhereClauses: string[] = [];
        ...
        if (typeId && authorId) {
            const topicsParams: unknown[] = [typeId, authorId];
            let paramIndex = 3;
    ...
    async getDocumentWithNeighbors(documentId: number | string): Promise<any | null> {
    ...
    async getTopics(spaceId: number | string | null, page: number = 1, limit: number = 15): Promise<any[]> {
