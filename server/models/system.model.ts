...
export interface DashboardStats {
    ...
    topSpaces: unknown[];
    topDharmaTalks: unknown[];
}

export const systemModel = {
    async getConfig(): Promise<SystemConfig | null> {
        let res = await pool.query('SELECT * FROM system_config LIMIT 1');

        // Auto-seed default row if table is empty
        if (res.rows.length === 0) {
            logger.info('[systemModel] No system_config row found. Seeding default row...');
            await pool.query(
                `INSERT INTO system_config (id, guest_message_limit, template, template_settings, system_keys, withdrawal_settings, platform_fee_percent)
                 VALUES (1, 10, 'default', '{}', '{}', '{}', 0)
                 ON CONFLICT (id) DO NOTHING`
            );
            res = await pool.query('SELECT * FROM system_config LIMIT 1');
        }
    ...
    async updateConfig(config: Partial<SystemConfig>): Promise<SystemConfig> {
        ...
        try {
            ...
            logger.error('Error in systemModel.updateConfig:', error);
            throw error;
        }
    },

    async getDashboardStats(spaceIds: (number | string)[] | null = null): Promise<DashboardStats> {
        const client = await pool.connect();
        try {
            const hasFilter = Array.isArray(spaceIds) && spaceIds.length > 0;
            const params: unknown[] = hasFilter ? [spaceIds] : [];
