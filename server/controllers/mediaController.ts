 ...
 */
export const getSpaceUploadDir = (spaceId: string | number, userId: string | number | null = null) => {
 ...
 */
const resolveUniqueFilename = async (dir: string, fileName: string) => {
...
export const mediaController = {
    async getMediaFiles(req: Request, res: Response) {
        ...
        try {
            ...
            mediaFiles.sort((a, b) => b.modifiedAt.getTime() - a.modifiedAt.getTime());

            // Pagination
            const page = Math.max(1, parseInt(req.query.page as string) || 1);
            const limit = Math.max(1, parseInt(req.query.limit as string) || 60);
            ...
            logger.error('Error reading media directory:', error);
            res.status(500).json({ message: 'Error fetching media files', error: (error instanceof Error ? error.message : String(error)) });
        }
    },
    ...
    async uploadMedia(req: Request, res: Response) {
        ...
        try {
            ...
            logger.error('Error saving media uploads:', error);
    ...
    async deleteMedia(req: Request, res: Response) {
        ...
        try {
            ...
            res.status(500).json({ message: 'Failed to delete files.', error: (error instanceof Error ? error.message : String(error)) });
        }
    }
};
