...
export const billingController = {
    // Pricing Plans
    async getPricingPlans(req: Request, res: Response) {
        try {
            const spaceId = req.query.spaceId ? parseInt(String(req.query.spaceId), 10) : null;
    ...
    async getAllTransactions(req: Request, res: Response) {
        try {
            ...
            if (isRootAdmin) {
                ...
                const spaceIdInt = spaceId ? parseInt(String(spaceId), 10) : null;
                if (spaceIdInt) {
                    const result = await billingModel.findTransactionsBySpaceId(spaceIdInt, { limit: 9999 });
                    return res.json(result.data || result);
                }
                return res.json(await billingModel.findAllTransactions());
            }

            // Space Owner: chỉ xem giao dịch của members trong space mình sở hữu
            const ownedSpaceRes = await pool.query('SELECT id FROM spaces WHERE user_id = $1', [user?.id]);
            if (ownedSpaceRes.rows.length > 0) {
                const spaceIds = ownedSpaceRes.rows.map((r: Record<string, unknown>) => r.id);
                const targetSpaceId = spaceId ? parseInt(String(spaceId), 10) : Number(spaceIds[0]);
                if (!spaceIds.includes(targetSpaceId)) {
                    return res.status(403).json({ message: 'Forbidden: You do not own this space.' });
                }
                const result = await billingModel.findTransactionsBySpaceId(targetSpaceId, { limit: 9999 });
                return res.json(result.data || result);
            }

            // Không phải admin, không phải space owner → chỉ xem của mình
            return res.json(await billingModel.findTransactionsByUserId(user?.id ? Number(user.id) : 0));
        } catch (error: unknown) {
    ...
    async getSpaceTransactions(req: Request, res: Response) {
        try {
    ...
    async addMeritsManually(req: Request, res: Response) {
    ...
    async purchaseSubscription(req: Request, res: Response) {
        ...
        try {
            ...
            res.status(400).json({ message: (error instanceof Error ? (error instanceof Error ? (error instanceof Error ? (error instanceof Error ? (error instanceof Error ? error.message : String(error)) : String(error)) : String(error)) : String(error)) : String(error)) || 'Lỗi khi mua gói.' });
        }
    },

    // Crypto (Mocked)
    // Crypto (Mocked) - REMOVED
    // async initiateMeritPurchase(req: Request, res: Response) {
    ...
    async getEnabledPaymentMethods(req: Request, res: Response) {
        try {
            ...
            res.status(500).json({ message: `Could not fetch payment methods: ${(error instanceof Error ? (error instanceof Error ? (error instanceof Error ? (error instanceof Error ? (error instanceof Error ? error.message : String(error)) : String(error)) : String(error)) : String(error)) : String(error))}` });
        }
    },
    ...
    //     } catch (error: unknown) {
    //         res.status(500).json({ message: `Failed to create payment intent: ${(error instanceof Error ? (error instanceof Error ? (error instanceof Error ? (error instanceof Error ? (error instanceof Error ? error.message : String(error)) : String(error)) : String(error)) : String(error)) : String(error))}` });
    //     }
    // },
    ...
    //         res.status(500).json({ message: `Failed to confirm payment and add merits: ${(error instanceof Error ? (error instanceof Error ? (error instanceof Error ? (error instanceof Error ? (error instanceof Error ? error.message : String(error)) : String(error)) : String(error)) : String(error)) : String(error))}` });
    //     }
    // },
    ...
    async verifyCheckoutSession(req: Request, res: Response) {
        ...
        try {
            ...
            if (donationType === 'offering' && spaceIdInt) {
                try {
                    ...
                    if (space) {
                        ...
                        if (userRes) {
                            await mailService.sendDonationSuccessEmail(userRes.email, userRes.name, meritsNum, space.name, String(paymentIntentId));
                        }
            ...
            res.status(500).json({ message: `Verification failed: ${(error instanceof Error ? (error instanceof Error ? (error instanceof Error ? (error instanceof Error ? (error instanceof Error ? error.message : String(error)) : String(error)) : String(error)) : String(error)) : String(error))}` });
        }
    },

    // NEW: Purchase AI daily limit boost with Merits
    async purchaseAiLimitWithMerits(req: Request, res: Response) {
        ...
        try {
            ...
            await billingModel.addDonateBonus(userId, plan.dailyLimitBonus || 0, plan.durationDays || 30);

            const updatedUser = await userModel.findById(userId);
            res.json(mapAndSanitizeUser(updatedUser));
        } catch (error: unknown) {
            res.status(500).json({ message: `Lỗi: ${(error instanceof Error ? (error instanceof Error ? (error instanceof Error ? (error instanceof Error ? (error instanceof Error ? error.message : String(error)) : String(error)) : String(error)) : String(error)) : String(error))}` });
        }
    },
    ...
    async processWithdrawalRequest(req: Request, res: Response) {
        const { id } = req.params;
    ...
    async createWithdrawalRequest(req: Request, res: Response) {
        ...
        const { systemModel } = await import('../models/system.model.js');
        const config = await systemModel.getConfig();
        const settings = ((config as Record<string, unknown>)?.withdrawalSettings as Record<string, unknown>) || { minWithdrawal: 50, holdDays: 5 };
        const minAmount = Number(settings.minWithdrawal) || 50;
        const holdDays = Number(settings.holdDays) || 5;
        ...
        try {
            if (spaceId) {
                ...
                if (((user as any).merits || 0) < amount) {
                    return res.status(400).json({ message: 'Số dư không đủ để rút.' });
                }
            ...
            res.status(500).json({ message: `Lỗi khi tạo yêu cầu rút tiền: ${(error instanceof Error ? (error instanceof Error ? (error instanceof Error ? (error instanceof Error ? (error instanceof Error ? error.message : String(error)) : String(error)) : String(error)) : String(error)) : String(error))}` });
        }
    },
    ...
    async exportTransactions(req: Request, res: Response) {
        try {
            const { spaceId } = req.query;
            const xlsxModule = await import('xlsx'); const xlsx = (xlsxModule as any).default || xlsxModule;
            ...
            const worksheetData = data.map((t: Record<string, unknown>) => ({
                Date: new Date(String(t.timestamp)).toLocaleString('vi-VN'),
                Type: t.type,
                Amount: t.merits,
            ...
            res.status(500).json({ message: 'Export failed: ' + (error instanceof Error ? (error instanceof Error ? (error instanceof Error ? (error instanceof Error ? (error instanceof Error ? error.message : String(error)) : String(error)) : String(error)) : String(error)) : String(error)) });
        }
    },

    async getSpaceEarningsStats(req: Request, res: Response) {
        try {
