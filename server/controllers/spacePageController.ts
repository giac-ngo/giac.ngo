...
async function renderShortcodes(html, space) {
    ...
    if (html.includes('{{dharma_talks}}')) {
        ...
        const talksHtml = talks.length
            ? `<div class="gc-dharma-talks">${talks.map((t: Record<string, unknown>) => `
                <div class="gc-card">
                    ${t.thumbnailUrl ? `<img src="${t.thumbnailUrl}" alt="${t.title}">` : ''}
                    <h3>${t.title || ''}</h3>
                    <p>${t.date ? new Date(String(t.date)).toLocaleDateString('vi-VN') : ''}</p>
                </div>`).join('')}</div>`
    ...
    if (html.includes('{{library}}')) {
        ...
        const libHtml = docs.length
            ? `<div class="gc-library">${docs.map((d: Record<string, unknown>) => `
                <div class="gc-card">
...
export const spacePageController = {
    ...
    async getPage(req: Request, res: Response) {
        try {
            ...
            logger.error('getPage error:', err);
    ...
    async uploadAsset(req: Request, res: Response) {
        try {
            ...
            logger.error('uploadAsset error:', err);
    ...
    async serveCustomDomainPage(req, res, space) {
        try {
            ...
            logger.error('serveCustomDomainPage error:', err);
     ...
     */
    async servePublicPage(req: Request, res: Response) {
        try {
            ...
            logger.error('servePublicPage error:', err);
            res.status(500).send('<html><body><h1>Server Error</h1></body></html>');
        }
    },
    ...
    async handleContactForm(req: Request, res: Response) {
        try {
            ...
            logger.error('handleContactForm error:', err);
