...
let _workingEmbedConfig: any = null;
...
async function generateGeminiEmbedding(text: string, apiKey: string, isQuery = false) {
    ...
    try {
        ...
        const embedModels = listData.models
            ?.filter((m: Record<string, unknown>) => (m.supportedGenerationMethods as any[])?.includes('embedContent'))
...
const getClassNameForModel = (modelType: string) => {
    const className = (WEAVIATE_CLASSES as any)[modelType];
...
const generateUuid = (sourceId: string | number, type: string) => {
...
const isSchemaCached = (className: string) => {
...
const setCacheEntry = (className: string, exists: boolean) => {
...
const weaviateService = {
    ...
    async _getScopedClient(modelType: string, apiKey: string) {
        ...
        try {
            ...
            throw new Error(`Could not connect to Weaviate. Check your config. Original error: ${(err instanceof Error ? (err instanceof Error ? (err instanceof Error ? (err instanceof Error ? err.message : String(err)) : String(err)) : String(err)) : String(err))}`);
        }
    },

    async ensureSchemaForModelType(modelType: string, apiKey: string) {
        ...
        try {
            ...
            if (existingClass) {
                ...
                if (existingClass.vectorizer === 'text2vec-palm') {
                    ...
                    if (cfg && ((cfg as any).apiEndpoint || (cfg as any).projectId === '')) {
            ...
            if (modelType === 'gpt') {
                ...
                console.log(`[SCHEMA] Creating ${className} with vectorizer=none (self-generated Gemini embeddings)`);
    ...
    async resetSchemaForModelType(modelType: string, apiKey: string) {
        const className = getClassNameForModel(modelType);
        if (!className) return;
        ...
        try {
            const schema = await client.schema.getter().do();
            const classExists = schema.classes?.some((c: Record<string, unknown>) => c.class === className);
    ...
    async indexData(modelType: string, dataSources: any[], apiKey: string, aiConfigId: string | number) {
        ...
        const toIndex = dataSources.filter((s: Record<string, unknown>) => !(s.indexedProviders as any[])?.includes(modelType)
        );
                    ...
                    if (counter >= batchSize) {
                        ...
                        for (let i = 0; i < results.length; i++) {
                            ...
                            if (item.result?.errors) {
                                const errMsg = (item.result.errors as any)[0]?.message || '';
    ...
    async search(modelType: string, aiConfigId: string | number, query: string, apiKey: string, limit = 5) {
        try {
            const client = await this._getScopedClient(modelType, apiKey);
            const className = getClassNameForModel(modelType);
            if (!className) return [];
            ...
            return (res as any).data.Get[className] || [];
        } catch (error: any) {
            // If embedding-001 error, mark class as broken to skip future searches
            if ((error instanceof Error ? error.message : String(error))?.includes('embedding-001') || error.message?.includes('text2vec-palm')) {
                const className = getClassNameForModel(modelType);
                if (className) {
                    brokenSchemas.add(className);
                    setCacheEntry(className, false); // invalidate schema cache
    ...
    async deleteDataByAiConfigId(modelType: string, aiConfigId: string | number, apiKey: string) {
        try {
            const client = await this._getScopedClient(modelType, apiKey);
            const className = getClassNameForModel(modelType);
            if (!className) return;
            ...
            const schema = await client.schema.getter().do();
            if (!schema.classes?.some((c: Record<string, unknown>) => c.class === className)) return;

            const result = await (client.batch as any).deleter()
                .withClassName(className)
    ...
    async deleteDataBySourceId(modelType: string, sourceId: string | number, sourceType: string, apiKey: string) {
        try {
            const client = await this._getScopedClient(modelType, apiKey);
            const className = getClassNameForModel(modelType);
            if (!className) return;
