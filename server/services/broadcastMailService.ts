 ...
 */
function buildDefaultEmailHtml(title: string, bodyHtml: string, recipientName = 'Thành viên') {
    return `<!DOCTYPE html>
<html lang="vi">
<head>
  <meta charset="UTF-8">
 ...
 */
function buildEmailHtml(title: string, body: string, recipientName = 'Thành viên', emailTemplate: string | null = null) {
    const bodyHtml = body
        .replace(/&/g, '&amp;')
        .replace(/</g, '&lt;')
        .replace(/>/g, '&gt;')
 ...
 */
export async function sendBulkEmail({ recipients, subject, title, body, emailTemplate = null, delayMs = 200 }: { recipients: Array<{email: string, name: string, spaceId?: number}>; subject: string; title: string; body: string; emailTemplate?: string | null; delayMs?: number }) {
    let sent = 0;
    let failed = 0;
    const errors = [];
    ...
    for (const recipient of recipients) {
        try {
            ...
            sent++;
            logger.info(`[Broadcast] Sent (${sent}/${recipients.length}): ${recipient.email}`);
        } catch (err: unknown) {
            failed++;
            errors.push({ email: recipient.email, error: (err instanceof Error ? (err instanceof Error ? (err instanceof Error ? (err instanceof Error ? (err instanceof Error ? (err instanceof Error ? (err instanceof Error ? (err instanceof Error ? (err instanceof Error ? (err instanceof Error ? err.message : String(err)) : String(err)) : String(err)) : String(err)) : String(err)) : String(err)) : String(err)) : String(err)) : String(err)) : String(err)) });
            logger.error(`[Broadcast] Failed: ${recipient.email} — ${(err instanceof Error ? (err instanceof Error ? (err instanceof Error ? (err instanceof Error ? err.message : String(err)) : String(err)) : String(err)) : String(err))}`);
        }
