...
async function getTransportAndFrom(options: Record<string, any> = {}) {
    ...
    if (!space || !space.smtp_host || !space.smtp_user) {
        logger.error("No SMTP config found for Space 1 or host:", host);
...
export const mailService = {
    ...
    async sendContactFormEmail(formData: Record<string, any>, options: Record<string, any> = {}) {
    ...
    async sendDonationSuccessEmail(to: string, userName: string, amount: number | string, spaceName: string, transactionId: string, options: Record<string, any> = {}) {
        ...
        try {
            ...
            logger.info('Donation success email sent to:', to);
        } catch (error: unknown) {
            logger.error('Error sending donation success email:', error);
        }
    },

    async sendDonationReceivedEmail(to: string, ownerName: string, amount: number | string, donorName: string, spaceName: string, note: string | null, options: Record<string, any> = {}) {
        ...
        try {
            ...
            logger.info('Donation received email sent to owner:', to);
        } catch (error: unknown) {
            logger.error('Error sending donation received email:', error);
        }
    },

    async sendWithdrawalStatusEmail(to: string, ownerName: string, amount: number | string, status: string, reason: string = '', options: Record<string, any> = {}) {
        ...
        try {
            ...
            logger.info(`Withdrawal status (${status}) email sent to:`, to);
        } catch (error: unknown) {
            logger.error('Error sending withdrawal status email:', error);
        }
    },

    async sendBroadcastEmail({ to, toName, subject, htmlBody, textBody, options = {} }: { to: string; toName?: string; subject: string; htmlBody?: string; textBody?: string; options?: Record<string, any> }) {
        try {
            const { transport, fromString } = await getTransportAndFrom(options);
            const mailOptions = {
                from: fromString,
