...
- **Hoàn thành**: 
  ...
  2. Kiến trúc: Di chuyển hệ thống lưu trữ cấu hình AI Keys (Gemini, ChatGPT, Grok) và Cài đặt Khách (Guest message limits) từ Global User Settings sang cấp độ `Space`. 
     ...
     - Refactor UI Admin `SpaceManagement.tsx` để nhập key cho từng Space.
- **Quyết định**: Áp dụng Fallback Hierarchy cho Key: `Space.apiKeys -> Owner.apiKeys -> System.systemKeys`. Giữ lại Personal Access Token ở settings cá nhân.
